        ...
        <div style={{ width:"100%", maxWidth:"720px", padding:"40px 24px 120px", position:"relative" }}>
          ...
          <div style={{ display:"flex", gap:"4px", flexWrap:"wrap", marginBottom:"16px", padding:"8px 12px", background:"#F8F8F6", borderRadius:"10px", border:"1px solid #EAEAEA", alignItems:"center" }}>
            <button type="button" title="굵게" onClick={() => {
              ...
              if (sel) { updateBlock(focusedId, v.slice(0,s)+"<b>"+sel+"</b>"+v.slice(e2)); }
            ...
            <button type="button" title="기울임" onClick={() => {
              ...
              const s=ta.selectionStart, e2=ta.selectionEnd, v=ta.value;
              const sel=v.slice(s,e2);
              if (sel) { updateBlock(focusedId, v.slice(0,s)+"<i>"+sel+"</i>"+v.slice(e2)); }
                    ...
                    {block.type === "text" && (
                      <textarea
                        ...
                        onMouseUp={e => {
                          ...
                          const scrollTop = edEl ? edEl.scrollTop : 0;
                          // textarea 내 선택된 텍스트의 줄 위치 추정
                          const lineH = 27;
                          const linesAbove = ta.value.slice(0, selStart).split("\n").length - 1;
                          const taScrollTop = ta.scrollTop || 0;
                          const relTop = taRect.top - edRect.top + linesAbove * lineH - taScrollTop - 36;
                          const relLeft = taRect.left - edRect.left;
                          setFloatBar({ top: Math.max(relTop, 4), left: Math.max(relLeft, 0), blockId: block.id, selStart, selEnd, selectedText: sel });
                        }}
                        onKeyUp={e => {
                          const ta = e.currentTarget;
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* HERO */}
      <div style={{ position:"relative", color:"#fff", padding:"52px 24px 44px", textAlign:"center", overflow:"hidden", minHeight:"240px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
